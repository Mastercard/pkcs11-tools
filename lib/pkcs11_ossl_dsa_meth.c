...
static int (*orig_dsa_sign) (EVP_PKEY_CTX *ctx,
			     unsigned char *sig, size_t *siglen,
			     const unsigned char *tbs, size_t tbslen) = NULL;



static int custom_dsa_sign( EVP_PKEY_CTX *ctx,
			    unsigned char *sig,
			    size_t *siglen,
    ...
    if( entered==true ) {
	return orig_dsa_sign(ctx, sig, siglen, tbs, tbslen);
    } else {
...
void pkcs11_dsa_method_setup()
{
    ...
    int (*orig_dsa_sign_init) (EVP_PKEY_CTX *ctx);
    ...
    EVP_PKEY_meth_set_sign(custom_dsamethod,
			   orig_dsa_sign_init, /* duplicate it, we don't change it */
			   custom_dsa_sign );  /* the new, customized method */
