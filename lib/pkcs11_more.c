...
static void more_pubk(EVP_PKEY* pk)
{
	...
	RSA_print(bio_stdout, EVP_PKEY_get1_RSA(pk), 0);
	break;

    case EVP_PKEY_DSA:
	DSA_print(bio_stdout, EVP_PKEY_get1_DSA(pk), 0);
	break;

    case EVP_PKEY_DH:
	DHparams_print(bio_stdout, EVP_PKEY_get1_DH(pk));
	break;

    case EVP_PKEY_EC:
	EC_KEY_print(bio_stdout, EVP_PKEY_get1_EC_KEY(pk), 0);
	break;
