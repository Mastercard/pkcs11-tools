...
|               |                               | `CKM_NSS_AES_KEY_WRAP` |              |                                  | also supports                |
|               |                               | `CKM_LUNA_AES_KW`      |              |                                  | vendor variants (NSS, Luna)  |
| `rfc5649`     | RFC5649, NIST SP.800.38F      | `CKM_AES_KEY_WRAP_PAD` | AES          | any key type                     | also supports                |
|               |                               | `CKM_AES_KEY_WRAP_KWP` |              |                                  | (PKCS#11 v3.0) and           |
|               |                               | `CKM_NSS_AES_KEY_WRAP_PAD` |          |                                  | vendor-specific variants     |
|               |                               | `CKM_LUNA_AES_KWP`     |              |                                  | (NSS, Luna)                  |
...
- `-a rfc3394`: private and secret key wrapping, as documented in RFC3394 and NIST.SP.800-38F, using `CKM_AES_KEY_WRAP`
  mechanism or equivalent vendor-specific (`CKM_NSS_AES_KEY_WRAP` on NSS; `CKM_LUNA_AES_KW` on Safenet/Thales Luna v7 when built with `--with-luna`).
  The standard mechanism is preferred automatically when several are advertised by the token.
- `-a rfc5649` or `-a rfc5649(flavour=<mech>)`: private and secret key wrapping, as documented in RFC5649 and NIST.SP.800-38F,
  using `CKM_AES_KEY_WRAP_PAD` mechanism or equivalent vendor-specific (`CKM_NSS_AES_KEY_WRAP_PAD` on NSS; `CKM_LUNA_AES_KWP` on Safenet/Thales Luna v7 when built with `--with-luna`).
  When several RFC5649-compatible mechanisms are advertised by the token, `CKM_AES_KEY_WRAP_PAD` (PKCS#11 v2.40) is preferred by default; otherwise the only mechanism available is used (so Luna v7 tokens transparently use `CKM_LUNA_AES_KWP`).
  Use `flavour=` to force a specific mechanism on wrap (and to record it in the wrapped-key file so the receiver picks the same one on unwrap):
    * `flavour=nss` selects NSS's `CKM_NSS_AES_KEY_WRAP_PAD` (note: NSS's implementation is **not** fully RFC5649-compliant)
