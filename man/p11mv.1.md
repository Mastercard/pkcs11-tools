% P11MV(1) | pkcs11-tools

# NAME

p11mv - move or rename a PKCS#11 token object
...
**p11mv** renames a PKCS#11 token object by changing its **CKA_LABEL**
attribute from *SOURCE* to *DESTINATION*. The object itself (and all other
...
label are renamed, one per class.

The command is interactive by default: when a matching source object is found,
the user is asked to confirm before the rename is performed. Use **-y** to
...
**-n**
:   Allow duplicate objects when the destination label already exists. Only
...
*SOURCE*
:   The current label of the object to rename. May be prefixed with `cert/`,
    `prvk/`, `pubk/`, `seck/`, or `data/` to restrict the rename to a single
    object class. When no prefix is given, all objects sharing the label are
    renamed.

*DESTINATION*
:   The new label to assign to the object(s).
...
Rename all objects labelled `wrapperkey` to `other-wrapperkey`, confirming
each rename interactively:
...
Rename only the private key, non-interactively:

    p11mv -y prvk/wrapperkey other-wrapperkey

Rename using a token label and a password read from a helper command:

    p11mv -t "my token" -p ":::exec:get-pin" wrapperkey other-wrapperkey

# SEE ALSO

**pkcs11-tools**(7), **pkcs11rc**(5), **p11ls**(1), **p11cp**(1),
